...
const htmlCardsTemplate = document.querySelector("#card-template").content;
/* ------------------------------ image preview ----------------------------- */
...
function submitAddPlaceForm(evt) {
	...
	const templateClone = htmlCardsTemplate.cloneNode(true);
	const listElement = templateClone.querySelector(".card");
	listElement.querySelector(".card__place-name").textContent = placeName;
	listElement.querySelector(".card__image").src = placeLink;
	listElement.querySelector(".card__place-favorite").addEventListener("click", function(evt) {
		evt.target.classList.toggle("card__place-favorite_active")
	});
	listElement.querySelector(".card__trash").addEventListener("click", () => {
		listElement.remove();
...
initialPlaces.forEach(function(card) {
	...
	const listElement = templateClone.querySelector(".card");
	//get title element
	listElement.querySelector(".card__place-name").textContent = card.name;
	const imgPopup = listElement.querySelector(".card__image").src = card.link;
	const captionName = listElement.querySelector(".card__place-name").textContent = card.name;
	listElement.querySelector(".card__place-favorite").addEventListener("click", function(evt) {
		evt.target.classList.toggle("card__place-favorite_active")
	});
	listElement.querySelector(".card__trash").addEventListener("click", () => {
	...
	listElement.querySelector(".card__image").addEventListener("click", () => {
